...
from game.state_machine import StateMachine, State
...
class Game:
    ...
    def update(self, dt: float):
        ...
        if self.debug_counter % 60 == 0:
            print(f"[DEBUG] State: {self.state}, Round: {self.round_state}")
    ...
    def update_fight(self, dt: float):
        ...
        if self.debug_counter % 60 == 0 and self.round_state == RoundState.FIGHT:
            print(f"[DEBUG] Keys: L={left} R={right} U={up} D={down} Block={block}")

        # 更新玩家1
        if self.player1:
            if self.round_state == RoundState.FIGHT:
                self.player1.handle_input(left, right, up, down, light_attack, heavy_attack, special, block)
            self.player1.update(dt, self.player2)

            if self.debug_counter % 60 == 0 and self.round_state == RoundState.FIGHT:
                print(f"[DEBUG] P1: x={self.player1.x:.0f}, vel_x={self.player1.vel_x:.1f}, blocking={self.player1.is_blocking}")
        ...
        if self.player2:
            ...
            if self.debug_counter % 60 == 0 and self.round_state == RoundState.FIGHT:
                print(f"[DEBUG] P2: x={self.player2.x:.0f}, vel_x={self.player2.vel_x:.1f}, blocking={self.player2.is_blocking}")
    ...
    def start_round(self):
        ...
        print(f"[DEBUG] Round started: P1 at {self.player1.x}, P2 at {self.player2.x}")
